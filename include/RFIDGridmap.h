...
class RFIDGridmap {
   ...
   */
    RFIDGridmap(std::string fileURI, double mapResolution, double gridResolution, bool debug);
   ...
   * @param antennaHeading angle between ellipsoid mayor axis and x axis (radians)
   * @param minX           shortest dist betwen F1 and ellipse (m.)
   * @param maxX           longest dist betwen F1 and ellipse (m.)
   */
    void addEllipse(double likelihood, double antennaX, double antennaY, double antennaHeading, double minX, double maxX);
  ...
  private:
    ...
    void addEllipse(grid_map::GridMap&  map_, std::string layerName, double likelihood, double antennaX, double antennaY, double antennaHeading, double minX, double maxX, bool debug);

    void addLine(grid_map::GridMap&  map_, std::string layerName,  double likelihood,double X1, double Y1, double X2, double Y2, bool debug);

    void addCircle(grid_map::GridMap&  map_, std::string layerName,  double likelihood,double X, double Y, double R, bool debug);

    void saveLayer( grid_map::GridMap map_, std::string layerName, std::string  fileURI, bool debug, std::string format);

    void createGrid(grid_map::GridMap&  map_, std::string layerName, std::string fileURI, double mapResolution, double gridResolution, bool debug, std::string global_frame, std::string format);

    void setPosition(grid_map::GridMap&  map_, std::string layerName, double val, double x, double y, bool debug);

    double getPosition(grid_map::GridMap&  map_, std::string layerName, double x, double y, bool debug);

    void setCell(grid_map::GridMap&  map_, std::string layerName, double val, int i,int j, bool debug);

    double getCell(grid_map::GridMap&  map_, std::string layerName, int i, int j, bool debug);
