...
namespace dummy {
    ...
    Map::Map() : map_grid_(vector<string>({"layer"})), nav_grid_(vector<string>({"layer"})),
                 planning_grid_(vector<string>({"layer"})), rfid_grid_(vector<string>({"layer"})) {


    }


    Map::Map(float plan_resolution, nav_msgs::OccupancyGrid occupancyGrid) : map_grid_(vector<string>({"layer"})),
        ...
        ROS_ASSERT_MSG(gridToPathGridScale >= 1,
                       "[Map.cpp@map] Planning resolution lower than navigation resolution = %3.3f",
                       gridToPathGridScale);
    ...
    Map::Map(std::ifstream &infile, float resolution) : map_grid_(vector<string>({"layer"})),
                                                        nav_grid_(vector<string>({"layer"})),
                                                        planning_grid_(vector<string>({"layer"})),
                                                        rfid_grid_(vector<string>({"layer"})) {
    ...
    Map::Map(float plan_resolution, float map_resolution, int width, int height, vector<int> data,
             geometry_msgs::Pose origin) : map_grid_(vector<string>({"layer"})), nav_grid_(vector<string>({"layer"})),
                                           planning_grid_(vector<string>({"layer"})),
                                           rfid_grid_(vector<string>({"layer"})) {
...
//        maxValue = 20;
    ...
    void Map::createMap(cv::Mat imageCV, geometry_msgs::Pose origin, std::string map_frame_id, double map_resolution) {
        ...
        tempMap.setGeometry(Length(numRows * map_resolution, numCols * map_resolution), map_resolution,
...
//                n_free++;
        ...
        ROS_DEBUG("[Map.cpp@encodeGrid] Encoded grid with %3.3f %% of free cells and %3.3f %% of occupied cells",
                  100.0 * n_free / (1.0 * total), 100.0 * n_obsts / (1.0 * total));
        if (n_others)
            ROS_WARN("[Map.cpp@encodeGrid] Found %3.3f %% undefined cells, CASTED to free space",
    ...
    cv::Mat Map::binarizeImage(cv::Mat mImg) {
        ...
        if (diffValues.size() > 2) {
            ...
            if (diffValues.size() != 2) {
                ROS_ERROR("[Map.cpp@createMap] After binarizing, we still have [%lu] different values: ",
                          diffValues.size());
...
//        cout << "maxValue(obstacle) = " << maxValue << ", minValue(free) = " << minValue << endl;
    ...
    void Map::updatePathPlanningGrid(int cellX_pp, int cellY_pp, int rangeInCells_pp, double power) {
        ...
        std::cout << "[Map.cpp@updatePathPlanningGrid] [numPathPlanningGridRows, numPathPlanningGridCols] = ["
                  << numPathPlanningGridRows << "," << numPathPlanningGridCols << "]" << endl;
        std::cout << "[Map.cpp@updatePathPlanningGrid] [cellX_pp, cellY_pp] = [" << cellX_pp << "," << cellY_pp << "]"
                  << endl;
        std::cout << "[Map.cpp@updatePathPlanningGrid] [minX_pp, maxX_pp] = [" << minX_pp << "," << maxX_pp << "]"
                  << endl;
        std::cout << "[Map.cpp@updatePathPlanningGrid] [minY_pp, maxY_pp] = [" << minY_pp << "," << maxY_pp << "]"
                  << endl;
...
//        }
        ...
        std::cout << "[Map.cpp@updatePathPlanningGrid] PlanningGrid scanned cells: " << counter_planning_grid_scanned
                  << endl;
    }


    void Map::printSubmapBoundaries(grid_map::Index startIndex, grid_map::Index bufferSize,
        ...
        ROS_DEBUG("Submap boundaries position: [%3.3f, %3.3f], [%3.3f, %3.3f], [%3.3f, %3.3f], [%3.3f, %3.3f]",
                  topLeftP.x(), topLeftP.y(), topRightP.x(), topRightP.y(), bottomRightP.x(), bottomRightP.y(),
                  bottomLeftP.x(), bottomLeftP.y());
    }
...
//      std::cout << "\n[Map.cpp@updatePathPlanningGrid] upper_left_pp = [" << upper_left_pp.x() << "," << upper_left_pp.y() << "]" << endl;
//      std::cout << "[Map.cpp@updatePathPlanningGrid] planningStartIndex = " << (planningStartIndex)(0)  << "," << (planningStartIndex)(1) << endl;

//      planningStartIndex = Position(posX - rangeInMeters, posY - rangeInMeters);
        // this is the number of navigation cells inside a planning cell
...
//      std::cout << "[Map.cpp@updatePathPlanningGrid] navGridResolution = " << nav_grid_.getResolution() <<
//          ", pathPlanningGridResolution =" << planning_grid_.getResolution()<< endl;
...
//        std::cout << "\n[Map.cpp@updatePathPlanningGrid] planning_iterator = [" << (*planning_iterator)(0) << "," << (*planning_iterator)(1) << "]" << endl;
//        std::cout << "[Map.cpp@updatePathPlanningGrid] position_pp = " << position_pp.x()  << "," << position_pp.y() << endl;
            numVistNavCellsPerPathCell = 0;

            // obtain the position of the upper-left  navigation cell INSIDE current planning cell
            upper_left_pp.x() = position_pp.x() - k;
            upper_left_pp.y() = position_pp.y() - k;
//        std::cout << "[Map.cpp@updatePathPlanningGrid] upper_left_pp = " << upper_left_pp.x()  << "," << upper_left_pp.y() << endl;
            // and corresponding index in nav_grid_
            nav_grid_.getIndex(upper_left_pp, navStartIndex);
...
//            (*nav_iterator)(0) << ", " << (*nav_iterator)(1) << endl;
    ...
    bool Map::getPosition(double &x, double &y, long i, long j, grid_map::GridMap *gm) {
        ...
        if (gm->getPosition(inIndex, inPose)) {
            ...
            ROS_ERROR("[1]Requested [%lu, %lu] index is outside grid boundaries: [0,0] - [%d, %d]", i, j,
                      gm->getSize()(0) - 1, gm->getSize()(1) - 1);
    ...
    float Map::getValue(long i, long j, const grid_map::GridMap *gm) const {
        ...
        if (gm->getPosition(ind, position)) {
            ...
            ROS_ERROR("[2]Requested [%lu, %lu] index is outside grid boundaries: [0,0] - [%d, %d]", i, j,
                      gm->getSize()(0) - 1, gm->getSize()(1) - 1);
            val = -1;
        }
        return val;
    ...
    float Map::getValue(geometry_msgs::PoseStamped ps, const grid_map::GridMap *gm) const {
        ...
        if (ps.header.frame_id != gm->getFrameId()) {
            ROS_ERROR("Pose given in different frame id: grid==[%s], point ==[%s]", ps.header.frame_id.c_str(),
                      gm->getFrameId().c_str());
            val = -1;
        } else {
...
//        cout << "Set value: " << value <<" at index: [" << i << "," << j <<"]" << endl;
            ...
            ROS_ERROR("[3]Requested [%lu, %lu] index is outside grid boundaries: [0,0] - [%d, %d]", i, j,
                      gm->getSize()(0) - 1, gm->getSize()(1) - 1);
        }
    }
...
//     cout << value << endl;
    ...
    void Map::printErrorReason(grid_map::Position point, const grid_map::GridMap *gm) const {
        ...
        gm->getPosition(bottomLeftI, bottomLeftP);
        ROS_ERROR(
                "Point [%3.3f, %3.3f] is outside grid boundaries: grid: [%3.3f, %3.3f], [%3.3f, %3.3f], [%3.3f, %3.3f], [%3.3f, %3.3f]",
                point.x(), point.y(), topLeftP.x(), topLeftP.y(), topRightP.x(), topRightP.y(), bottomRightP.x(),
                bottomRightP.y(), bottomLeftP.x(), bottomLeftP.y());
    }
    ...
    void Map::printGridData(std::string grid_name, const grid_map::GridMap *gm) {
        ...
        ROS_DEBUG("[Map.cpp@printGridData] Grid Center [%3.3f %3.3f] m. ", gm->getPosition().x(),
                  gm->getPosition().y());
        ...
        ROS_DEBUG(" \t\t topLeft cell: [%d, %d] == [%3.3f, %3.3f] m.", topLeftI.x(), topLeftI.y(), topLeftP.x(),
                  topLeftP.y());
        ROS_DEBUG(" \t\t topRight cell: [%d, %d] == [%3.3f, %3.3f] m.", topRightI.x(), topRightI.y(), topRightP.x(),
                  topRightP.y());
        ROS_DEBUG(" \t\t bottomRight cell: [%d, %d] == [%3.3f, %3.3f] m.", bottomRightI.x(), bottomRightI.y(),
                  bottomRightP.x(), bottomRightP.y());
        ROS_DEBUG(" \t\t bottomLeft cell: [%d, %d] == [%3.3f, %3.3f] m.", bottomLeftI.x(), bottomLeftI.y(),
                  bottomLeftP.x(), bottomLeftP.y());

        countCells(&n_obsts, &n_free, &n_vist, &n_others, gm);
        total = n_obsts + n_free + n_vist + n_others;
        ROS_DEBUG(
                "[Map.cpp@printGridData] Grid has %3.3f %% of free cells, %3.3f %% of occupied cells and  %3.3f %% of visited cells",
                100.0 * n_free / (1.0 * total), 100.0 * n_obsts / (1.0 * total), 100.0 * n_vist / (1.0 * total));
        if (n_others)
 ...
 */
    ...
    int Map::isCandidate_inner(long i, long j, int mode) {
        ...
        if (!planning_grid_.getPosition(grid_map::Index(i, j), position_pp)) {
            ROS_ERROR("[4]Requested [%lu, %lu] index is outside path planning grid boundaries: [0,0] - [%d, %d]", i, j,
                      mapSize(0) - 1, mapSize(1) - 1);
            candidate = 0;
        } else {
            grid_map::SubmapIterator iterator(planning_grid_, submapStartIndex, submapBufferSize);
            ROS_DEBUG("Iterating over a submap size [%d x %d]]", iterator.getSubmapSize()(0),
                      iterator.getSubmapSize()(1));
    ...
    void
    Map::findCandidatePositions(double pos_X_m, double pos_Y_m, double heading_rad, double FOV_rad, double range_m) {
        findCandidatePositions_inner(1, pos_X_m, pos_Y_m, heading_rad, FOV_rad, range_m);
    }

    void
    Map::findCandidatePositions2(double pos_X_m, double pos_Y_m, double heading_rad, double FOV_rad, double range_m) {
        findCandidatePositions_inner(2, pos_X_m, pos_Y_m, heading_rad, FOV_rad, range_m);
    }

    void Map::findCandidatePositions_inner(int mode, double pos_X_m, double pos_Y_m, double heading_rad, double FOV_rad,
                                           double range_m) {
...
//                                cout <<"[map.cpp@findCandidatePositions] Pos: " << rayPos(0) << "," << rayPos(1) << endl;
                        ...
                        if (isPathPlanningGridValueObst(rayIndex)) {
                            hit = true;
                            ROS_DEBUG(
                                    "[map.cpp@findCandidatePositions] HIT! cell [%d, %d]- [%3.3f m., %3.3f m.] -  Hit point: [%d, %d]- [%3.3f m., %3.3f m.]",
                                    candidateIndex(0), candidateIndex(1), candidatePos(0), candidatePos(1), rayIndex(0),
                                    rayIndex(1), rayPos(0), rayPos(1));
//                                  cout << "HIT! cell" << endl;
                            break;
                        }
                    }
...
//        cout << "[map.cpp@findCandidatePositions] CandidatePosition found: " << edgePoints.size() << endl;
    ...
    Map::getSensingTime(double pos_X_m, double pos_Y_m, double heading_rad, double FOV_rad, double range_m) {
        grid_map::Index candidateIndex, rayIndex;
        grid_map::Position candidatePos, rayPos;
        double rel_y, rel_x, rel_a;
        grid_map::Position centerPos(pos_X_m, pos_Y_m);
    ...
    int Map::performSensingOperation(double pos_X_m, double pos_Y_m, double heading_rad, double FOV_rad, double range_m,
                                     double minAngle_rad, double maxAngle_rad) {
                ...
                if (std::abs(rel_a) <= (FOV_rad / 2)) {
                    ...
                    for (grid_map::LineIterator lin_iterator(nav_grid_, candidatePos, centerPos);
                        ...
                        if (isGridValueObst(*nav_iterator)) {
                            hit = true;
                            ROS_DEBUG(
                                    "[map.cpp@getInformationGain] HIT! cell [%d, %d]- [%3.3f m., %3.3f m.] -  Hit point: [%d, %d]- [%3.3f m., %3.3f m.]",
                                    candidateIndex(0), candidateIndex(1), candidatePos(0), candidatePos(1), rayIndex(0),
                                    rayIndex(1), rayPos(0), rayPos(1));
                            break;
                        }
...
//          cout << "[map.cpp@getInformationGain] Totals free cells in nav_grid area: " << freeCells << endl;
    ...
    void Map::calculateInfoGainSensingTime(double pos_X_m, double pos_Y_m, double heading_rad, double FOV_rad,
                                           double range_m) {
    ...
    Map::CellValue Map::toCellValue(float floatVal, std::string who) const {
        ...
        if (floatVal == 0.0) {
            ...
            ROS_FATAL(
                    "[Map@toCellValue (%s)] INVALID CASTING REQUESTED... (%3.3f does not equal to any valid cell value)",
                    who.c_str(), floatVal);
            value = Map::CellValue::FREE;
        }
