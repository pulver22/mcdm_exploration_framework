...
EvaluationRecords *
MCDMFunction::evaluateFrontiers(const std::list<Pose> &frontiers, dummy::Map *map, double threshold, ros::ServiceClient* path_client)
{
...
string MCDMFunction::getEncodedKey(Pose &p, int value) {
  ...
  if (value == 0) {
    key = to_string(p.getX()) + "/" + to_string(p.getY()) + "/" + to_string(p.getOrientation()) + "/" +
          to_string(p.getRange()) + "/" + to_string(p.getFOV());
  } else if (value == 1) {
