...
double SensingTimeCriterion::SensingTimeCriterion::evaluate(Pose &p, dummy::Map *map, ros::ServiceClient *path_client) {
  NewRay ray;
  double sensingTime;
  double angle;
...
//    cout << " [sensingTimeCriterion.cpp@Evaluate][2] [posX, posY] = [" << x_meter << "," << y_meter << "]" << endl;
  ...
  p.setScanAngles(map->getSensingTime(p.getX(), p.getY(), p.getOrientation(), p.getFOV(), p.getRange()));
  float minPhi = (float)p.getScanAngles().first;
  float maxPhi = (float)p.getScanAngles().second;
//  std::cout << "Orientation: " << orientation << ", FOV: " << p.getFOV() << endl;
//  std::cout << "startingPhi " << startingPhi << " endingPhi " << endingPhi << std::endl;
...
//  angle = maxPhi - minPhi;
  ...
  if (isnan(angle) or angle < 0.0001) angle = 0;
