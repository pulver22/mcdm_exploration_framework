...
double InformationGainCriterion::evaluate(Pose &p, dummy::Map *map,
  ...
  /*
      ...
      // intersection point between the laser sensor (at the edge) and the
     vertical/horizontal segment passing from
      // the considered cell
      //int* intersection;
      ...
      // orientation == 90 means the robot is looking toward the upper border of
     the map
      if( orientation == 90 ){
          //minSensedX is the robot position minus the radius projection on the
     x axis; the same also for maxSensedX but with plus
          //instead of minus
          ...
          //maxSensedY is the robot position, since the index grow towards the
     bottom
          maxSensedY = py;
          //normalize the sensed cell to not allow to sense outside the map
     edges
          ...
          for(int i=minSensedY  ; i< maxSensedY; i++){
              for (int j = minSensedX; j< maxSensedX; j++){
                  // if the considered cell is before the robot position
     (relating to the xaxis)
                  if( j < px ){
                      // calculate the intersection point between the vertical
     segment passing for the cell and the radius at the left edge
                      int* intersection =intersect(i,j,minSensedX,minSensedY,p);
                      // if the j-coordinate of the considered cell is higher (y
     grow toward the bottom and x toward right) than the intersection's one,
                      //it means that the cell is not insidie the sensed area,
     so break this loop and look for the following cell
                      if ( i > intersection[1]){
                          continue;
                      }
                  }else{
                  ...
                  // if the cell is inside the sensed area, we can increase our
     counter
                  sensedArea++;
          ...
          for(int i=minSensedY  ; i< maxSensedY; i++){
              for (int j = minSensedX; j< maxSensedX; j++){
                  if( j < px ){
                      int* intersection =
     intersect(i,j,minSensedX,minSensedY,p);
                      if ( i < intersection[1]){
                          continue;
                      }
                  }else{
                      int* intersection =
     intersect(i,j,maxSensedX,minSensedY,p);
                      if (i < intersection[1]){
                      continue;
                      }
                  }
     ...
     intersect(i,j,maxSensedX,minSensedY,p);
                      ...
                      int* intersection =
     intersect(i,j,maxSensedX,maxSensedY,p);
                      if (i < intersection[1]){
                      continue;
                      }
                  }
     ...
     intersect(i,j,minSensedX,minSensedY,p);
                      ...
                      int* intersection =
     intersect(i,j,minSensedX,maxSensedY,p);
                      if (i < intersection[1]){
                      continue;
                      }
                  }
      ...
      // the information gain is caluclated as the subtraction between the
     sensed area and the occupied one
  ...
  */
  ...
  //    cout << " [informationGainCriterion.cpp@Evaluate][2] [posX, posY] = ["
...
//  cout << "   [informationGainCriterion.cpp@Evaluate] unExploredMap = " << unExploredMap << endl;
  ...
  if (unExploredMap >= 0.5) {
      ...
      cout<< "No other candidate position that guarantee information gain" <<
  endl;
  }*/
  Criterion::insertEvaluation(p, unExploredMap);
...
/*
int* InformationGainCriterion::intersect(int p1x, int p1y, int p2x, int p2y,
Pose p)
{
    float m1, m2, q1, q2;
