...
void updatePathMetrics(int *count, Pose *target, Pose *previous, string actualPose, list<Pose> *nearCandidates,
                       ...
                       list<pair<float, float>> *posToEsclude, vector<string> *history, int encodedKeyValue,
                       Astar *astar, long *numConfiguration,
                       double *totalAngle, double *travelledDistance, int *numOfTurning, double scanAngle,
                       ros::ServiceClient *path_client, bool backTracking);

list<Pose> cleanHistory(vector<string> *history, EvaluationRecords *record_history);

void
printResult(long newSensedCells, long totalFreeCells, double precision, long numConfiguration, double travelledDistance,
            int numOfTurning, double totalAngle, double totalScanTime);

void
showMarkerandNavigate(Pose target, ros::Publisher *marker_pub, nav_msgs::GetPlan *path, ros::ServiceClient *path_client,
                      list<Pose> *tabuList, std::list<std::pair<float, float> > *posToEsclude);

// ROS varies
void move(float x, float y, float orientation, float time_travel, list<Pose> *tabuList,
...
//  }
    ...
    while (disConnected) {
        ...
        if (map_service_client_.call(srv_map)) {
            ...
            costmap_update_sub = nh.subscribe<map_msgs::OccupancyGridUpdate>("move_base/global_costmap/costmap_updates",
                                                                             10, update_callback);
            disConnected = false;
        } else {
            r.sleep();
...
//      history.push_back(function.getEncodedKey(target, 1));
            ...
            do {
                ...
                cout << "\n============================================" << endl;
                cout << "New iteration, position: " << target.getX() << "," << target.getY() << endl;
                cout << "Graph size: " << graph2.size() << endl;
                if (graph2.size() == 0 and count != 0) break;
...
//          if (count != 0)
//          {
//            move(target.getX(), target.getY(), target.getOrientation(), 10.0);
//          }
...
//          newSensedCells = sensedCells + ray.performSensingOperation ( &map, 5.0, 3.0, 90, 180, 3, -1.8, 1.8 );
                    ...
                    float x = target.getX();
                    float y = target.getY();
                    float orientation = roundf(target.getOrientation() * 100) / 100;;  // cast orientation in [0, 360]
...
//          cout << "[pure_navigation.cpp@main](x,y) = (" << x << "," << y << ")" << endl;
...
//              max_pan_angle = 0;
                    ...
                    if (count != 0 && (target.isEqual(invertedInitial) || target.isEqual(eastInitial) ||
                    ...
                    if (candidatePosition.size() == 0) {
                        ...
                        if (graph2.size() > 1) {
                            ...
                            cout << "[BT]No significand position reachable. Come back to previous position" << endl;
...
//                            showMarkerandNavigate(target, &marker_pub, &path, &path_client, &tabuList,
//                                                  &posToEsclude);
                            scan = false;
                        }
                            ...
                            printResult(newSensedCells, totalFreeCells, precision, numConfiguration, travelledDistance,
                                        numOfTurning,
                        ...
                        for (it; it != candidatePosition.end(); it++) {
                            Pose p1 = Pose((*it).first, (*it).second, roundf(0 * 100) / 100, range, FOV);
                            Pose p2 = Pose((*it).first, (*it).second, roundf(M_PI / 4 * 100) / 100, range, FOV);
                            Pose p3 = Pose((*it).first, (*it).second, roundf(M_PI / 2 * 100) / 100, range, FOV);
                            Pose p4 = Pose((*it).first, (*it).second, roundf(3 * M_PI / 4 * 100) / 100, range, FOV);
                            Pose p5 = Pose((*it).first, (*it).second, roundf(M_PI * 100) / 100, range, FOV);
                            Pose p6 = Pose((*it).first, (*it).second, roundf(5 * M_PI / 4 * 100) / 100, range, FOV);
                            Pose p7 = Pose((*it).first, (*it).second, roundf(3 * M_PI / 2 * 100) / 100, range, FOV);
                            Pose p8 = Pose((*it).first, (*it).second, roundf(7 * M_PI / 4 * 100) / 100, range, FOV);
...
//                                cout << "2" << endl;
                                ...
                                encodedKeyValue = 1;
                                backTracking = false;
                                updatePathMetrics(&count, &target, &previous, actualPose, &nearCandidates, &graph2,
                                                  &map, &function,
                                                  &tabuList, &posToEsclude, &history, encodedKeyValue, &astar,
                                                  &numConfiguration,
...
//                                cout << "3" << endl;
                                ...
                                if (graph2.at(graph2.size() - 1).second.size() != 0) {
                                    cout << "[BT1 - Tabulist]There are visible cells but the selected one is already "
                                            "(or cannot be) explored!Come back to second best position from the previous position" << endl;
...
//                        cout << "Inside the while" << endl;
                                    ...
                                    cout << "[BT2 - Tabulist]There are visible cells but the selected one is already "
                                            "explored! Come back to two position ago" << endl;
                            ...
                            cout << "[BT3] There are no visible cells so come back to previous position in the graph"
                                    " structure:" << endl;
                            ...
                            target = record->getPoseFromEncoding(targetString);
                            cout << "--> " << function.getEncodedKey(target, 0) << endl;
                            cout << "Previous: " << function.getEncodedKey(previous, 2) << endl;
...
//            if ( ! contains ( tabuList,target ) )
                        if (!containsPos(posToEsclude, targetPos)) {
                            ...
                            encodedKeyValue = 1;
                            backTracking = true;
                            updatePathMetrics(&count, &target, &previous, actualPose, &nearCandidates, &graph2, &map,
                                              &function,
                                              &tabuList, &posToEsclude, &history, encodedKeyValue, &astar,
                                              &numConfiguration,
                                              &totalAngle, &travelledDistance, &numOfTurning, scanAngle, &path_client,
                                              backTracking);
                            showMarkerandNavigate(target, &marker_pub, &path, &path_client, &tabuList,
                                                  &posToEsclude);
                            ...
                            cout << "[BT-MODE4] Go back to the best frontiers from the previous positions in the graph" << endl;
                            cout << "       " << function.getEncodedKey(target, 0) << endl;
                        }
...
//                                                  &path_client, backTracking);
                        ...
                        while(1){
                            if (graph2.size()!= 0){  // If there are still position on which doing backtracking
                                if ((graph2.at(graph2.size() - 1).second).size() > 0)  // If there are still frontiers from the current position
                                {
                                    string targetString = graph2.at(graph2.size() - 1).first;
                                    nearCandidates = graph2.at(graph2.size() - 1).second;
                                    target = record->getPoseFromEncoding(targetString);
...
//                                          backTracking);
                                    // Leave backtracking
                                    btMode = true;
                                    cout << "[BT-MODE3] There are no candidate frontiers in the record. Go back to previous positions in the graph" << endl;
                                    cout << "       " << targetString << endl;
                                    break;
                                } else{
                                    cout << "[BT-MODE3] No more frontiers with the associate position in the graph" << endl;
                                    graph2.pop_back();
                                }
                            } else{
                                cout << "[BT-MODE3] Graph2 is empty. Navigation is finished" << endl;
                                break;
                            }
...
bool containsPos(std::list<std::pair<float, float> > positionEscluded, std::pair<float, float> p) {
    ...
    auto findIter = std::find(positionEscluded.begin(), positionEscluded.end(), p);
...
void cleanPossibleDestination2(std::list<Pose> *possibleDestinations, Pose &p) {
    MCDMFunction function;
//  cout << "[pure_navigation.cpp@cleanPossibleDestination2] I want to remove "<< function.getEncodedKey(p,0) << endl;
//  cout << "[pure_navigation.cpp@cleanPossibleDestination2] Size possible destination: " << possibleDestinations->size() << endl;
...
//    cout << "[pure_navigation.cpp@cleanPossibleDestination2] EncodedKey:" <<  function.getEncodedKey(*findIter,0) << "\n" << endl;
        possibleDestinations->erase(findIter);
    }
//  else cout<< "[pure_navigation.cpp@cleanPossibleDestination2] Cell not found\n" << endl;
...
double calculateScanTime(double scanAngle) {
    return (-7.2847174296449998e-006 * scanAngle * scanAngle * scanAngle +
            2.2131847908245512e-003 * scanAngle * scanAngle + 1.5987873410233613e-001 * scanAngle + 10);
}
...
//  cout << function->getEncodedKey ( *target,1 ) << endl;
    ...
    posToEsclude->push_back(make_pair(roundf(((*target).getX() * 100) / 100), roundf((*target).getY() * 100 / 100)));
...
//    }
        ...
        ROS_INFO("[pure_navigation@updatePathMetrics] Path_finding Service call failed! ");
    }
...
void move(float x, float y, float orientation, float time_travel, list<Pose> *tabuList,
    ...
    if (orientation > M_PI) _orientation = orientation - 2 * M_PI;
...
//  cout << "   [pure_navigation.cpp@move] [map]goal: (" << x << "," << y << ") with orientation: " << _orientation << "(" << _orientation * 180 / M_PI << ")"<< endl;
    ac.sendGoal(goal);
...
//  cout << "     [pure_navigation.cpp@move] Waiting for " << time_travel << " seconds" << endl;
    ...
    if (ac.getState() == actionlib::SimpleClientGoalState::SUCCEEDED) {
        ...
        ROS_INFO(
                "[pure_navigation.cpp@move] The base failed to move, adding this pose to the Tabulist and posToEsclude");
...
//  cout << "   New goal in map: X = " << p.point.x << ", Y = " << p.point.y << " m. "<<endl;
    ...
    path->request.goal.pose.position.y = target.getY();
    path->request.goal.pose.orientation.w = 1;
//  cout << "[pure_navigation.cpp@showMarkerandNavigate] [PATH-START] = (" << path->request.start.pose.position.x << ", " << path->request.start.pose.position.y << ")" << endl;
//  cout << "[pure_navigation.cpp@showMarkerandNavigate] [PATH-GOAL] = (" << path->request.goal.pose.position.x << ", " << path->request.goal.pose.position.y << ")" << endl;
    bool path_srv_call = path_client->call(*path);
    float path_len;
    if (path_srv_call) {
        // calculate path length
        path_len = getPathLen(path->response.plan.poses);
        if (path_len < 1e3) {
//            ROS_INFO("[pure_navigation.cpp@showMarkerandNavigate] Path len is [%3.3f m.]", path_len);
        } else {
//            ROS_INFO("[pure_navigation.cpp@showMarkerandNavigate] Path len is infinite");
            path_len = 1000;
        }
    } else {
        ROS_INFO("[pure_navigation.cpp@showMarkerandNavigate] Service call failed! ");
        path_len = 1000;
    }

    float time_travel = 2 * path_len / min_robot_speed;
//    cout << "[pure_navigation.cpp@showMarkerandNavigate] Target is at " << path_len << " m from the robot" << endl;

    move(p.point.x, p.point.y, roundf(target.getOrientation() * 100) / 100, time_travel, tabuList,
