    ...
    if (map_service_client_.call(srv_map)) {
      ...
      if (costmapReceived == 1) {
        ...
        */
        ...
        dummy::Map map = dummy::Map(resolution, costresolution, costwidth,
        ...
        */
        ...
        // cout <<start_pose.pose.orientation.x <<","<<
        // start_pose.pose.orientation.y <<","<< start_pose.pose.orientation.z<<
        // ","<< start_pose.pose.orientation.w <<endl;

        tfScalar angle = 2 * atan2(quat[2], quat[3]);

        cout << "Initial position in the map frame:" << initX << "," << initY
             << " with orientation :" << angle << endl;
        ...
        if (resolution >= 0 && resolution < 1 && resolution != costresolution) {
          ...
          // cout << "[BEFORE]Initial position in the image frame: " <<
          // pose.getX() * costresolution<< "," << (map.getPathPlanningNumRows()
          // - (long)pose.getY())*costresolution << endl;
          Pose initialPose =
              Pose(map.getPathPlanningNumRows() - (long)pose.getY(),
                   (long)pose.getX(), initOrientation, initRange, initFov);
          target = initialPose;
          previous = initialPose;
          cout << "[AFTER]Initial position in the image frame: "
               << target.getY() * costresolution << ","
               << target.getX() * costresolution << endl;
        } else {
          // 1mx1m
          // cout << "[BEFORE]Initial position in the image frame: " <<
          // pose.getX()<< "," << map.getPathPlanningNumRows() -
          // (long)pose.getY() << endl;
          ...
          previous = initialPose;
          cout << "[AFTER]Initial position in the image frame: "
               << target.getY() << "," << target.getX() << endl;
        }
        Pose invertedInitial = createFromInitialPose(
            initX, initY, initOrientation, 180, initRange, initFov);
        Pose eastInitial = createFromInitialPose(initX, initY, initOrientation,
                                                 90, initRange, initFov);
        Pose westInitial = createFromInitialPose(initX, initY, initOrientation,
                                                 270, initRange, initFov);

        //----------------- PRINT INITIAL POSITION
        // ATTENTION: doesn't work! ...anyway the initial position is
        ...
        if (resolution != 0) {
          p.point.x = (map.getPathPlanningNumRows() - target.getX()) +
                      0.5;                   //* costresolution;
          p.point.y = (target.getY()) + 0.5; //* costresolution;
        } else {
          p.point.x =
              (map.getPathPlanningNumRows() - target.getX()) * costresolution;
        ...
        // amount of time the robot should do nothing for scanning the
        // environment ( final value expressed in second)
        ...
        while (sensedCells < precision * totalFreeCells - 200) {
          ...
          /*
          for(int i = 0; i < map.getPathPlanningNumRows(); ++i)
          {
          for(int j = 0; j < map.getPathPlanningNumCols(); ++j)
              {
                  std::cout << map.getPathPlanningGridValue(i,j);
              }
          std::cout << std::endl;
          }
          std::cout << std::endl;
          */
          ...
          if (candidatePosition.size() == 0) {
            ...
            // NOTE: TAKE THIS BRANCH IF THERE ARE NO CANDIDATE POSITION FROM
            // THE ACTUAL ONE
            ...
            // cout << "Graph dimension : " << graph2.size() << endl;
            // cout << "Candidate position: " << candidatePosition.size() <<
            // endl;
            ...
            if (record->size() != 0) {
              ...
              if (contains(tabuList, target) == false) {
                ...
                cout << "Goal in image: " << target.getY() << "," <<
                target.getX() << endl;
                tf::Vector3 goal_pose =
                tf::Vector3(target.getY(),target.getX(),0.0);
                //cout << "1)" <<goal_pose.getX() << "," << goal_pose.getY() <<
                endl;

                goal_pose = goal_pose ;//* resolution;
                //cout << "2)" <<goal_pose.getX() << "," << goal_pose.getY() <<
                endl;

                //goal_pose = goal_pose.operator-=(vecImageToMap);
                //cout << "3)" <<goal_pose.getX() << "," << goal_pose.getY() <<
                endl;

                cout << "Goal in map: " << goal_pose.getX() <<","<<
                goal_pose.getY() << endl;
                */
                ...
                if (resolution >= 0 && resolution < 1 &&
                  ...
                  p.point.x =
                      (map.getNumGridRows() - target.getX()) * costresolution;
                  p.point.y = (target.getY()) * costresolution;
                ...
                cout << "New goal in map: X = " << p.point.x
                     << ", Y = " << p.point.y << endl;
                ...
                double orientZ =
                    (double)(target.getOrientation() * PI / (2 * 180));
                double orientW =
                    (double)(target.getOrientation() * PI / (2 * 180));
                if (resolution != 0) {
                ...
                cout << "[BT - Tabulist]There are visible cells but the "
                        "selected one is already explored!Come back to "
              ...
              if (!target.isEqual(previous)) {
                ...
                if (graph2.size() == 0) {
                  cout << "No other possibilities to do backtracking on "
                          "previous positions since there are no more position "
                          "in the graph"
                       << endl;
                  break;
                }
                ...
                graph2.pop_back();
                previous = target;
                cout << "[BT]There are no visible cells so come back to "
                        "previous position"
                     << endl;
                cout << "[BT]Cell already explored!Come back to previous "
                        "position"
                     << endl;
                history.push_back(function.getEncodedKey(target, 2));
                cout << "New target: x = " << target.getY()
...
void gasDetection() {
  ...
  */
  ...
  if (client1.call(srvSweep)) {
    ROS_INFO("Gas detection in progress ... <%.2f~%.2f,%.2f>", min_pan_angle,
             max_pan_angle, tilt_angle);
  } else {
